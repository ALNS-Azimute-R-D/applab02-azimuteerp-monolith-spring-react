...
describe('Activity e2e test', () => {
  ...
  const activitySample = { name: 'boohoo slushy deskill', activationStatus: 'INACTIVE' };
  ...
  beforeEach(() => {
    ...
    cy.authenticatedRequest({
      ...
      body: { acronym: 'timbale when among', name: 'apropos unto um', description: 'why vent', handlerClazzName: 'while woot ew' },
    }).then(({ body }) => {
      typeOfActivity = body;
    });
  });
  ...
  describe('new Activity page', () => {
    ...
    it('should create an instance of Activity', () => {
      cy.get(`[data-cy="name"]`).type('orderly gadzooks');
      cy.get(`[data-cy="name"]`).should('have.value', 'orderly gadzooks');

      cy.get(`[data-cy="shortDescription"]`).type('minty meridian upright');
      cy.get(`[data-cy="shortDescription"]`).should('have.value', 'minty meridian upright');

      cy.get(`[data-cy="fullDescription"]`).type('awful');
      cy.get(`[data-cy="fullDescription"]`).should('have.value', 'awful');

      cy.get(`[data-cy="mainGoals"]`).type('um regarding recess');
      cy.get(`[data-cy="mainGoals"]`).should('have.value', 'um regarding recess');

      cy.get(`[data-cy="estimatedDurationTime"]`).type('PT38M');
      cy.get(`[data-cy="estimatedDurationTime"]`).blur();
      cy.get(`[data-cy="estimatedDurationTime"]`).should('have.value', 'PT38M');

      cy.get(`[data-cy="lastPerformedDate"]`).type('2024-06-09');
      cy.get(`[data-cy="lastPerformedDate"]`).blur();
      cy.get(`[data-cy="lastPerformedDate"]`).should('have.value', '2024-06-09');

      cy.get(`[data-cy="createdAt"]`).type('2024-06-09T07:58');
      cy.get(`[data-cy="createdAt"]`).blur();
      cy.get(`[data-cy="createdAt"]`).should('have.value', '2024-06-09T07:58');
