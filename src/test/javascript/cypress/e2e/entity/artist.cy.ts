...
describe('Artist e2e test', () => {
  ...
  const artistSample = { acronym: 'handsome daub infuriate', publicName: 'cylindrical contend besides' };
  ...
  beforeEach(() => {
    ...
    cy.authenticatedRequest({
      ...
      body: { acronym: 'uh-huh spouse limp', name: 'bench a ugh', description: 'over' },
    }).then(({ body }) => {
      typeOfArtmedia = body;
    });
    // create an instance at the required relationship entity:
    cy.authenticatedRequest({
      ...
      body: { acronym: 'meanwhile joyously', name: 'following', description: 'those warmhearted' },
    }).then(({ body }) => {
      typeOfArtist = body;
    });
  });
  ...
  describe('new Artist page', () => {
    ...
    it('should create an instance of Artist', () => {
      cy.get(`[data-cy="acronym"]`).type('tambour');
      cy.get(`[data-cy="acronym"]`).should('have.value', 'tambour');

      cy.get(`[data-cy="publicName"]`).type('astonishing');
      cy.get(`[data-cy="publicName"]`).should('have.value', 'astonishing');

      cy.get(`[data-cy="realName"]`).type('whenever for');
      cy.get(`[data-cy="realName"]`).should('have.value', 'whenever for');

      cy.get(`[data-cy="biographyDetailsJSON"]`).type('graduate graduation');
      cy.get(`[data-cy="biographyDetailsJSON"]`).should('have.value', 'graduate graduation');
