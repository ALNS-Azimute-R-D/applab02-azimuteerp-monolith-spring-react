...
describe('OrganizationDomain e2e test', () => {
  ...
  // const organizationDomainSample = {"domainAcronym":"helpfully now","name":"atop","isVerified":true,"activationStatus":"INACTIVE"};
  ...
  beforeEach(() => {
    ...
    cy.authenticatedRequest({
      ...
      body: {"acronym":"though once","businessCode":"hm flaky","hierarchicalLevel":"need","name":"phew despite throughout","description":"gladly shady by","businessHandlerClazz":"sympathetically","mainContactPersonDetailsJSON":"geography","technicalEnvironmentsDetailsJSON":"that wholly","customAttributesDetailsJSON":"under","organizationStatus":"IN_FAILURE","activationStatus":"INVALID","logoImg":"Li4vZmFrZS1kYXRhL2Jsb2IvaGlwc3Rlci5wbmc=","logoImgContentType":"unknown"},
    }).then(({ body }) => {
      organization = body;
    });
  });
  ...
  describe('OrganizationDomain page', () => {
    ...
    describe('with existing value', () => {
      ...
      it.skip('last delete button click should delete instance of OrganizationDomain', () => {
        cy.intercept('GET', '/api/organization-domains/*').as('dialogDeleteRequest');
  ...
  describe('new OrganizationDomain page', () => {
    ...
    it.skip('should create an instance of OrganizationDomain', () => {
      cy.get(`[data-cy="domainAcronym"]`).type('enthrall');
      cy.get(`[data-cy="domainAcronym"]`).should('have.value', 'enthrall');

      cy.get(`[data-cy="name"]`).type('past tergiversate');
      cy.get(`[data-cy="name"]`).should('have.value', 'past tergiversate');
      ...
      cy.get(`[data-cy="businessHandlerClazz"]`).type('notice of brr');
      cy.get(`[data-cy="businessHandlerClazz"]`).should('have.value', 'notice of brr');
