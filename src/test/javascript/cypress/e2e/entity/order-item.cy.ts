...
describe('OrderItem e2e test', () => {
  ...
  // const orderItemSample = {"quantity":17092,"totalPrice":25252.07,"status":"AVAILABLE"};
  ...
  beforeEach(() => {
    ...
    cy.authenticatedRequest({
      method: 'POST',
      url: '/api/articles',
      body: {"inventoryProductId":17117,"skuCode":"except whose iridescence","customName":"aside instantly","customDescription":"algorithm","priceValue":16916.96,"itemSize":"M","activationStatus":"ACTIVE"},
    }).then(({ body }) => {
      article = body;
    });
    // create an instance at the required relationship entity:
    cy.authenticatedRequest({
      method: 'POST',
      url: '/api/orders',
      body: {"businessCode":"bite-sized","placedDate":"2024-06-09T04:55:53.187Z","totalTaxValue":19886.78,"totalDueValue":31916.6,"status":"COMPLETED","estimatedDeliveryDate":"2024-06-09T14:25:43.562Z","activationStatus":"INACTIVE"},
    }).then(({ body }) => {
      order = body;
    });
  });
  ...
  describe('new OrderItem page', () => {
    ...
    it.skip('should create an instance of OrderItem', () => {
      cy.get(`[data-cy="quantity"]`).type('1916');
      cy.get(`[data-cy="quantity"]`).should('have.value', '1916');

      cy.get(`[data-cy="totalPrice"]`).type('27571.14');
      cy.get(`[data-cy="totalPrice"]`).should('have.value', '27571.14');
