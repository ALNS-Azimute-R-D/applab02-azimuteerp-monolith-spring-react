...
describe('PaymentGateway e2e test', () => {
  ...
  const paymentGatewaySample = { aliasCode: 'serpentine', description: 'gee toreador', activationStatus: 'PENDENT' };
  ...
  describe('new PaymentGateway page', () => {
    ...
    it('should create an instance of PaymentGateway', () => {
      cy.get(`[data-cy="aliasCode"]`).type('gosh');
      cy.get(`[data-cy="aliasCode"]`).should('have.value', 'gosh');

      cy.get(`[data-cy="description"]`).type('within geez');
      cy.get(`[data-cy="description"]`).should('have.value', 'within geez');

      cy.get(`[data-cy="businessHandlerClazz"]`).type('from');
      cy.get(`[data-cy="businessHandlerClazz"]`).should('have.value', 'from');
