...
describe('ScheduledActivity e2e test', () => {
  ...
  const scheduledActivitySample = { startTime: '2024-06-09T01:03:09.702Z', activationStatus: 'INVALID' };
  ...
  describe('ScheduledActivity page', () => {
    ...
    describe('with existing value', () => {
      ...
      it('last delete button click should delete instance of ScheduledActivity', () => {
        cy.intercept('GET', '/api/scheduled-activities/*').as('dialogDeleteRequest');
  ...
  describe('new ScheduledActivity page', () => {
    ...
    it('should create an instance of ScheduledActivity', () => {
      cy.get(`[data-cy="customizedName"]`).type('e-mail absentmindedly direct');
      cy.get(`[data-cy="customizedName"]`).should('have.value', 'e-mail absentmindedly direct');

      cy.get(`[data-cy="startTime"]`).type('2024-06-08T16:46');
      cy.get(`[data-cy="startTime"]`).blur();
      cy.get(`[data-cy="startTime"]`).should('have.value', '2024-06-08T16:46');

      cy.get(`[data-cy="endTime"]`).type('2024-06-09T09:29');
      cy.get(`[data-cy="endTime"]`).blur();
      cy.get(`[data-cy="endTime"]`).should('have.value', '2024-06-09T09:29');

      cy.get(`[data-cy="numberOfAttendees"]`).type('16988');
      cy.get(`[data-cy="numberOfAttendees"]`).should('have.value', '16988');

      cy.get(`[data-cy="averageEvaluationInStars"]`).type('7555');
      cy.get(`[data-cy="averageEvaluationInStars"]`).should('have.value', '7555');

      cy.get(`[data-cy="customAttributtesDetailsJSON"]`).type('inasmuch face on');
      cy.get(`[data-cy="customAttributtesDetailsJSON"]`).should('have.value', 'inasmuch face on');
