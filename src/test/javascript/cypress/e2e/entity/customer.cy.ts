...
describe('Customer e2e test', () => {
  ...
  // const customerSample = {"customerBusinessCode":"wedding","fullname":"unless embezzle directive","customerStatus":"UNDER_EVALUATION","activationStatus":"ON_HOLD"};
  ...
  beforeEach(() => {
    ...
    cy.authenticatedRequest({
      method: 'POST',
      url: '/api/people',
      body: {"firstname":"brr filthy filthy","lastname":"requite not","fullname":"concrete lope undertaker","birthDate":"2024-06-09","gender":"OTHER","codeBI":"highly whose negligi","codeNIF":"field","streetAddress":"who iron agonize","houseNumber":"that since","locationName":"linear who brush","postalCode":"ouch enfo","mainEmail":"Ee**w@y#X?!.08","landPhoneNumber":"the oof","mobilePhoneNumber":"through yowza","occupation":"professionalise when","preferredLanguage":"thron","usernameInOAuth2":"yawning shun","userIdInOAuth2":"weakly","customAttributesDetailsJSON":"mimosa lively through","activationStatus":"INVALID","avatarImg":"Li4vZmFrZS1kYXRhL2Jsb2IvaGlwc3Rlci5wbmc=","avatarImgContentType":"unknown"},
    }).then(({ body }) => {
      person = body;
    });
  });
  ...
  describe('new Customer page', () => {
    ...
    it.skip('should create an instance of Customer', () => {
      cy.get(`[data-cy="customerBusinessCode"]`).type('embryo whose');
      cy.get(`[data-cy="customerBusinessCode"]`).should('have.value', 'embryo whose');

      cy.get(`[data-cy="fullname"]`).type('uphold');
      cy.get(`[data-cy="fullname"]`).should('have.value', 'uphold');

      cy.get(`[data-cy="customAttributesDetailsJSON"]`).type('loosely quaff');
      cy.get(`[data-cy="customAttributesDetailsJSON"]`).should('have.value', 'loosely quaff');

      cy.get(`[data-cy="customerStatus"]`).select('WORKING');
