...
describe('Program e2e test', () => {
  ...
  const programSample = { name: 'colorfully', activationStatus: 'ON_HOLD' };
  ...
  describe('Program page', () => {
    ...
    describe('with existing value', () => {
      ...
      it('last delete button click should delete instance of Program', () => {
        cy.intercept('GET', '/api/programs/*').as('dialogDeleteRequest');
  ...
  describe('new Program page', () => {
    ...
    it('should create an instance of Program', () => {
      cy.get(`[data-cy="acronym"]`).type('duh vainly weekly');
      cy.get(`[data-cy="acronym"]`).should('have.value', 'duh vainly weekly');

      cy.get(`[data-cy="name"]`).type('lap capitalize yowza');
      cy.get(`[data-cy="name"]`).should('have.value', 'lap capitalize yowza');

      cy.get(`[data-cy="description"]`).type('apud');
      cy.get(`[data-cy="description"]`).should('have.value', 'apud');

      cy.get(`[data-cy="fullDescription"]`).type('through mostly livid');
      cy.get(`[data-cy="fullDescription"]`).should('have.value', 'through mostly livid');

      cy.get(`[data-cy="targetPublic"]`).type('after ack');
      cy.get(`[data-cy="targetPublic"]`).should('have.value', 'after ack');
