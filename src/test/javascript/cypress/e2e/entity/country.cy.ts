...
describe('Country e2e test', () => {
  ...
  const countrySample = { acronym: 'exa', name: 'unruly', continent: 'EUROPE' };
  ...
  describe('new Country page', () => {
    ...
    it('should create an instance of Country', () => {
      cy.get(`[data-cy="acronym"]`).type('mea');
      cy.get(`[data-cy="acronym"]`).should('have.value', 'mea');

      cy.get(`[data-cy="name"]`).type('left eek');
      cy.get(`[data-cy="name"]`).should('have.value', 'left eek');

      cy.get(`[data-cy="continent"]`).select('CENTRAL_AMERICA');
