...
describe('TicketPurchased e2e test', () => {
  ...
  describe('TicketPurchased page', () => {
    ...
    describe('with existing value', () => {
      ...
      it('last delete button click should delete instance of TicketPurchased', () => {
        cy.intercept('GET', '/api/ticket-purchaseds/*').as('dialogDeleteRequest');
  ...
  describe('new TicketPurchased page', () => {
    ...
    it('should create an instance of TicketPurchased', () => {
      cy.get(`[data-cy="buyingCode"]`).type('past upon bitterly');
      cy.get(`[data-cy="buyingCode"]`).should('have.value', 'past upon bitterly');

      cy.get(`[data-cy="duePaymentDate"]`).type('2024-06-08T20:46');
      cy.get(`[data-cy="duePaymentDate"]`).blur();
      cy.get(`[data-cy="duePaymentDate"]`).should('have.value', '2024-06-08T20:46');

      cy.get(`[data-cy="amountOfTickets"]`).type('13833');
      cy.get(`[data-cy="amountOfTickets"]`).should('have.value', '13833');

      cy.get(`[data-cy="taxValue"]`).type('189.63');
      cy.get(`[data-cy="taxValue"]`).should('have.value', '189.63');

      cy.get(`[data-cy="ticketValue"]`).type('16491.72');
      cy.get(`[data-cy="ticketValue"]`).should('have.value', '16491.72');

      cy.get(`[data-cy="acquiredSeatsNumbers"]`).type('concerning');
      cy.get(`[data-cy="acquiredSeatsNumbers"]`).should('have.value', 'concerning');

      cy.get(`[data-cy="description"]`).type('whenever along owlishly');
      cy.get(`[data-cy="description"]`).should('have.value', 'whenever along owlishly');
