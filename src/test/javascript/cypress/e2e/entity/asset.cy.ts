...
describe('Asset e2e test', () => {
  ...
  const assetSample = { name: 'feng yum nor', activationStatus: 'PENDENT' };
  ...
  beforeEach(() => {
    ...
    cy.authenticatedRequest({
      ...
      body: {
        acronym: 'cook worth',
        name: 'summon hospitalize',
        description: 'atop yowza where',
        handlerClazzName: 'whenever um however',
        customAttributesDetailsJSON: 'vivaciously questionable agile',
      },
    }).then(({ body }) => {
      assetType = body;
    });
  ...
  describe('new Asset page', () => {
    ...
    it('should create an instance of Asset', () => {
      cy.get(`[data-cy="name"]`).type('smell');
      cy.get(`[data-cy="name"]`).should('have.value', 'smell');

      cy.get(`[data-cy="storageTypeUsed"]`).select('BLOB_IN_DB');

      cy.get(`[data-cy="fullFilenamePath"]`).type('ethyl since beneath');
      cy.get(`[data-cy="fullFilenamePath"]`).should('have.value', 'ethyl since beneath');
