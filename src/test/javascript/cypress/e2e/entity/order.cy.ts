...
describe('Order e2e test', () => {
  ...
  // const orderSample = {"businessCode":"backup following ins","placedDate":"2024-06-08T21:22:44.308Z","status":"COMPLETED","activationStatus":"INACTIVE"};
  ...
  beforeEach(() => {
    ...
    cy.authenticatedRequest({
      method: 'POST',
      url: '/api/customers',
      body: {"customerBusinessCode":"where","fullname":"rebuild midst","customAttributesDetailsJSON":"blah smith youthfully","customerStatus":"WORKING","activationStatus":"ON_HOLD"},
    }).then(({ body }) => {
      customer = body;
    });
  });
  ...
  describe('new Order page', () => {
    ...
    it.skip('should create an instance of Order', () => {
      cy.get(`[data-cy="businessCode"]`).type('geez');
      cy.get(`[data-cy="businessCode"]`).should('have.value', 'geez');

      cy.get(`[data-cy="placedDate"]`).type('2024-06-09T05:43');
      cy.get(`[data-cy="placedDate"]`).blur();
      cy.get(`[data-cy="placedDate"]`).should('have.value', '2024-06-09T05:43');

      cy.get(`[data-cy="totalTaxValue"]`).type('11180.15');
      cy.get(`[data-cy="totalTaxValue"]`).should('have.value', '11180.15');

      cy.get(`[data-cy="totalDueValue"]`).type('21176.36');
      cy.get(`[data-cy="totalDueValue"]`).should('have.value', '21176.36');

      cy.get(`[data-cy="status"]`).select('PENDING');

      cy.get(`[data-cy="estimatedDeliveryDate"]`).type('2024-06-09T00:35');
      cy.get(`[data-cy="estimatedDeliveryDate"]`).blur();
      cy.get(`[data-cy="estimatedDeliveryDate"]`).should('have.value', '2024-06-09T00:35');
