...
describe('AssetType e2e test', () => {
  ...
  const assetTypeSample = { name: 'deficit exhausted wrong' };
  ...
  describe('AssetType page', () => {
    ...
    describe('with existing value', () => {
      ...
      it('last delete button click should delete instance of AssetType', () => {
        cy.intercept('GET', '/api/asset-types/*').as('dialogDeleteRequest');
  ...
  describe('new AssetType page', () => {
    ...
    it('should create an instance of AssetType', () => {
      cy.get(`[data-cy="acronym"]`).type('stained modulo raw');
      cy.get(`[data-cy="acronym"]`).should('have.value', 'stained modulo raw');

      cy.get(`[data-cy="name"]`).type('instead ton');
      cy.get(`[data-cy="name"]`).should('have.value', 'instead ton');

      cy.get(`[data-cy="description"]`).type('amongst');
      cy.get(`[data-cy="description"]`).should('have.value', 'amongst');

      cy.get(`[data-cy="handlerClazzName"]`).type('deeply gee barring');
      cy.get(`[data-cy="handlerClazzName"]`).should('have.value', 'deeply gee barring');

      cy.get(`[data-cy="customAttributesDetailsJSON"]`).type('through so absent');
      cy.get(`[data-cy="customAttributesDetailsJSON"]`).should('have.value', 'through so absent');
