...
describe('District e2e test', () => {
  ...
  // const districtSample = {"acronym":"sale una","name":"somber what site"};
  ...
  beforeEach(() => {
    ...
    cy.authenticatedRequest({
      method: 'POST',
      url: '/api/town-cities',
      body: {"acronym":"lest amu","name":"agreement foal snarling","geoPolygonArea":"Li4vZmFrZS1kYXRhL2Jsb2IvaGlwc3Rlci5wbmc=","geoPolygonAreaContentType":"unknown"},
    }).then(({ body }) => {
      townCity = body;
    });
  });
  ...
  describe('new District page', () => {
    ...
    it.skip('should create an instance of District', () => {
      cy.get(`[data-cy="acronym"]`).type('ack amon');
      cy.get(`[data-cy="acronym"]`).should('have.value', 'ack amon');

      cy.get(`[data-cy="name"]`).type('yowza jaywalk');
      cy.get(`[data-cy="name"]`).should('have.value', 'yowza jaywalk');
