...
describe('Article e2e test', () => {
  ...
  const articleSample = { inventoryProductId: 17457, itemSize: 'L', activationStatus: 'BLOCKED' };
  ...
  describe('Article page', () => {
    ...
    describe('with existing value', () => {
      ...
      it('last delete button click should delete instance of Article', () => {
        cy.intercept('GET', '/api/articles/*').as('dialogDeleteRequest');
  ...
  describe('new Article page', () => {
    ...
    it('should create an instance of Article', () => {
      cy.get(`[data-cy="inventoryProductId"]`).type('10857');
      cy.get(`[data-cy="inventoryProductId"]`).should('have.value', '10857');

      cy.get(`[data-cy="skuCode"]`).type('lonely');
      cy.get(`[data-cy="skuCode"]`).should('have.value', 'lonely');

      cy.get(`[data-cy="customName"]`).type('now uh-huh');
      cy.get(`[data-cy="customName"]`).should('have.value', 'now uh-huh');

      cy.get(`[data-cy="customDescription"]`).type('sure-footed');
      cy.get(`[data-cy="customDescription"]`).should('have.value', 'sure-footed');

      cy.get(`[data-cy="priceValue"]`).type('6615.28');
      cy.get(`[data-cy="priceValue"]`).should('have.value', '6615.28');
