...
describe('BusinessUnit e2e test', () => {
  ...
  // const businessUnitSample = {"acronym":"circa reappoint","name":"ruler now","activationStatus":"INVALID"};
  ...
  beforeEach(() => {
    ...
    cy.authenticatedRequest({
      ...
      body: {"acronym":"dead confront automa","businessCode":"carefully gun","hierarchicalLevel":"lest offensively","name":"yahoo huzzah yieldingly","description":"saddle","businessHandlerClazz":"ram","mainContactPersonDetailsJSON":"afflict flick","technicalEnvironmentsDetailsJSON":"meanwhile behind furthermore","customAttributesDetailsJSON":"primary miniature","organizationStatus":"READY_TO_START","activationStatus":"ACTIVE","logoImg":"Li4vZmFrZS1kYXRhL2Jsb2IvaGlwc3Rlci5wbmc=","logoImgContentType":"unknown"},
    }).then(({ body }) => {
      organization = body;
    });
  });
  ...
  describe('BusinessUnit page', () => {
    ...
    describe('with existing value', () => {
      ...
      it.skip('last delete button click should delete instance of BusinessUnit', () => {
        cy.intercept('GET', '/api/business-units/*').as('dialogDeleteRequest');
  ...
  describe('new BusinessUnit page', () => {
    ...
    it.skip('should create an instance of BusinessUnit', () => {
      cy.get(`[data-cy="acronym"]`).type('whereas excepting');
      cy.get(`[data-cy="acronym"]`).should('have.value', 'whereas excepting');

      cy.get(`[data-cy="hierarchicalLevel"]`).type('in troubled reason');
      cy.get(`[data-cy="hierarchicalLevel"]`).should('have.value', 'in troubled reason');

      cy.get(`[data-cy="name"]`).type('whether yuck');
      cy.get(`[data-cy="name"]`).should('have.value', 'whether yuck');
