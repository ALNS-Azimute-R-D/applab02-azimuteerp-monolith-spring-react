...
describe('OrganizationAttribute e2e test', () => {
  ...
  beforeEach(() => {
    ...
    cy.authenticatedRequest({
      ...
      body: {"acronym":"usefully regarding","businessCode":"embarrassed and","hierarchicalLevel":"stream aboard salute","name":"under picnic","description":"although meh apropos","businessHandlerClazz":"underneath yahoo","mainContactPersonDetailsJSON":"aftershave easily","technicalEnvironmentsDetailsJSON":"batch after","customAttributesDetailsJSON":"excommunicate","organizationStatus":"PENDENT","activationStatus":"INVALID","logoImg":"Li4vZmFrZS1kYXRhL2Jsb2IvaGlwc3Rlci5wbmc=","logoImgContentType":"unknown"},
    }).then(({ body }) => {
      organization = body;
    });
  });
  ...
  describe('OrganizationAttribute page', () => {
    ...
    describe('with existing value', () => {
      ...
      it.skip('last delete button click should delete instance of OrganizationAttribute', () => {
        cy.intercept('GET', '/api/organization-attributes/*').as('dialogDeleteRequest');
  ...
  describe('new OrganizationAttribute page', () => {
    ...
    it.skip('should create an instance of OrganizationAttribute', () => {
      cy.get(`[data-cy="attributeName"]`).type('till footstool now');
      cy.get(`[data-cy="attributeName"]`).should('have.value', 'till footstool now');

      cy.get(`[data-cy="attributeValue"]`).type('striped sans fraud');
      cy.get(`[data-cy="attributeValue"]`).should('have.value', 'striped sans fraud');
