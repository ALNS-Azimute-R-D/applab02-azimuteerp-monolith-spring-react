...
describe('TypeOfEvent e2e test', () => {
  ...
  const typeOfEventSample = { name: 'supposing' };
  ...
  describe('TypeOfEvent page', () => {
    ...
    describe('with existing value', () => {
      ...
      it('last delete button click should delete instance of TypeOfEvent', () => {
        cy.intercept('GET', '/api/type-of-events/*').as('dialogDeleteRequest');
  ...
  describe('new TypeOfEvent page', () => {
    ...
    it('should create an instance of TypeOfEvent', () => {
      cy.get(`[data-cy="acronym"]`).type('excluding honored');
      cy.get(`[data-cy="acronym"]`).should('have.value', 'excluding honored');

      cy.get(`[data-cy="name"]`).type('shakily bah');
      cy.get(`[data-cy="name"]`).should('have.value', 'shakily bah');

      cy.get(`[data-cy="description"]`).type('fabric');
      cy.get(`[data-cy="description"]`).should('have.value', 'fabric');

      cy.get(`[data-cy="handlerClazzName"]`).type('helplessly');
      cy.get(`[data-cy="handlerClazzName"]`).should('have.value', 'helplessly');
