...
describe('TypeOfVenue e2e test', () => {
  ...
  const typeOfVenueSample = { name: 'deeply riddle which' };
  ...
  describe('TypeOfVenue page', () => {
    ...
    describe('with existing value', () => {
      ...
      it('last delete button click should delete instance of TypeOfVenue', () => {
        cy.intercept('GET', '/api/type-of-venues/*').as('dialogDeleteRequest');
  ...
  describe('new TypeOfVenue page', () => {
    ...
    it('should create an instance of TypeOfVenue', () => {
      cy.get(`[data-cy="acronym"]`).type('reclamation');
      cy.get(`[data-cy="acronym"]`).should('have.value', 'reclamation');

      cy.get(`[data-cy="name"]`).type('willfully');
      cy.get(`[data-cy="name"]`).should('have.value', 'willfully');

      cy.get(`[data-cy="description"]`).type('across angrily supposing');
      cy.get(`[data-cy="description"]`).should('have.value', 'across angrily supposing');

      cy.get(`[data-cy="handlerClazzName"]`).type('downturn vastly simplistic');
      cy.get(`[data-cy="handlerClazzName"]`).should('have.value', 'downturn vastly simplistic');
