...
describe('Artwork e2e test', () => {
  ...
  const artworkSample = { artworkTitle: 'creepy rude' };
  ...
  beforeEach(() => {
    ...
    cy.authenticatedRequest({
      ...
      body: { acronym: 'starter forceful', name: 'qua zowie', description: 'civilisation blackberry hm' },
    }).then(({ body }) => {
      typeOfArtmedia = body;
    });
  });
  ...
  describe('new Artwork page', () => {
    ...
    it('should create an instance of Artwork', () => {
      cy.get(`[data-cy="artworkTitle"]`).type('exciting yippee');
      cy.get(`[data-cy="artworkTitle"]`).should('have.value', 'exciting yippee');

      cy.get(`[data-cy="productionYear"]`).type('1858');
      cy.get(`[data-cy="productionYear"]`).should('have.value', '1858');

      cy.get(`[data-cy="seasonNumber"]`).type('6511');
      cy.get(`[data-cy="seasonNumber"]`).should('have.value', '6511');

      cy.get(`[data-cy="episodeOrSequenceNumber"]`).type('18605');
      cy.get(`[data-cy="episodeOrSequenceNumber"]`).should('have.value', '18605');

      cy.get(`[data-cy="registerIdInIMDB"]`).type('avaricious athwart');
      cy.get(`[data-cy="registerIdInIMDB"]`).should('have.value', 'avaricious athwart');

      cy.get(`[data-cy="assetsCollectionUUID"]`).type('jovially whole');
      cy.get(`[data-cy="assetsCollectionUUID"]`).should('have.value', 'jovially whole');

      cy.get(`[data-cy="contentDetailsJSON"]`).type('provided as raisin');
      cy.get(`[data-cy="contentDetailsJSON"]`).should('have.value', 'provided as raisin');
