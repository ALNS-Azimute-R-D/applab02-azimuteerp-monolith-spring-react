...
describe('TypeOfArtmedia e2e test', () => {
  ...
  const typeOfArtmediaSample = { acronym: 'um satin retrace', name: 'teeter', description: 'unbearably thread' };
  ...
  describe('TypeOfArtmedia page', () => {
    ...
    describe('with existing value', () => {
      ...
      it('last delete button click should delete instance of TypeOfArtmedia', () => {
        cy.intercept('GET', '/api/type-of-artmedias/*').as('dialogDeleteRequest');
  ...
  describe('new TypeOfArtmedia page', () => {
    ...
    it('should create an instance of TypeOfArtmedia', () => {
      cy.get(`[data-cy="acronym"]`).type('mmm');
      cy.get(`[data-cy="acronym"]`).should('have.value', 'mmm');

      cy.get(`[data-cy="name"]`).type('thrift considering yippee');
      cy.get(`[data-cy="name"]`).should('have.value', 'thrift considering yippee');

      cy.get(`[data-cy="description"]`).type('bogus aw');
      cy.get(`[data-cy="description"]`).should('have.value', 'bogus aw');
