...
describe('RawAssetProcTmp e2e test', () => {
  ...
  const rawAssetProcTmpSample = { name: 'unkempt' };
  ...
  beforeEach(() => {
    ...
    cy.authenticatedRequest({
      ...
      url: '/api/asset-types',
      body: {
        acronym: 'anguished grizzled',
        name: 'old-fashioned bah though',
        description: 'covariate espy misty',
        handlerClazzName: 'whenever',
        customAttributesDetailsJSON: 'suspiciously',
      },
    }).then(({ body }) => {
      assetType = body;
    });
  ...
  describe('RawAssetProcTmp page', () => {
    ...
    describe('with existing value', () => {
      ...
      it('last delete button click should delete instance of RawAssetProcTmp', () => {
        cy.intercept('GET', '/api/raw-asset-proc-tmps/*').as('dialogDeleteRequest');
  ...
  describe('new RawAssetProcTmp page', () => {
    ...
    it('should create an instance of RawAssetProcTmp', () => {
      ...
      cy.get(`[data-cy="statusRawProcessing"]`).select('FAILURE');

      cy.get(`[data-cy="fullFilenamePath"]`).type('naturally lest worth');
      cy.get(`[data-cy="fullFilenamePath"]`).should('have.value', 'naturally lest worth');

      cy.setFieldImageAsBytesOfEntity('assetRawContentAsBlob', 'integration-test.png', 'image/png');

      cy.get(`[data-cy="customAttributesDetailsJSON"]`).type('psst');
      cy.get(`[data-cy="customAttributesDetailsJSON"]`).should('have.value', 'psst');
