...
describe('Venue e2e test', () => {
  ...
  const venueSample = { name: 'ick hmph with' };
  ...
  beforeEach(() => {
    ...
    cy.authenticatedRequest({
      ...
      body: { acronym: 'hence yuck though', name: 'duh', description: 'yippee', handlerClazzName: 'how' },
    }).then(({ body }) => {
      typeOfVenue = body;
    });
  });
  ...
  describe('Venue page', () => {
    ...
    describe('with existing value', () => {
      ...
      it('last delete button click should delete instance of Venue', () => {
        cy.intercept('GET', '/api/venues/*').as('dialogDeleteRequest');
  ...
  describe('new Venue page', () => {
    ...
    it('should create an instance of Venue', () => {
      cy.get(`[data-cy="acronym"]`).type('neatly famously');
      cy.get(`[data-cy="acronym"]`).should('have.value', 'neatly famously');

      cy.get(`[data-cy="name"]`).type('splice through');
      cy.get(`[data-cy="name"]`).should('have.value', 'splice through');

      cy.get(`[data-cy="description"]`).type('zealous clearly');
      cy.get(`[data-cy="description"]`).should('have.value', 'zealous clearly');
