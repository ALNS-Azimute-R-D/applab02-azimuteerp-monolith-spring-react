...
describe('Payment e2e test', () => {
  ...
  const paymentSample = {
    installmentNumber: 21069,
    paymentDueDate: '2024-06-08T20:00:43.572Z',
    paymentPaidDate: '2024-06-09T14:27:49.224Z',
    paymentAmount: 30368.5,
    typeOfPayment: 'CASH',
    statusPayment: 'OPEN',
  ...
  beforeEach(() => {
    ...
    cy.authenticatedRequest({
      ...
      body: {
        aliasCode: 'frenetically psst oo',
        description: 'seat yahoo nor',
        businessHandlerClazz: 'or oleo',
  ...
  describe('new Payment page', () => {
    ...
    it('should create an instance of Payment', () => {
      cy.get(`[data-cy="installmentNumber"]`).type('17074');
      cy.get(`[data-cy="installmentNumber"]`).should('have.value', '17074');

      cy.get(`[data-cy="paymentDueDate"]`).type('2024-06-08T21:07');
      cy.get(`[data-cy="paymentDueDate"]`).blur();
      cy.get(`[data-cy="paymentDueDate"]`).should('have.value', '2024-06-08T21:07');

      cy.get(`[data-cy="paymentPaidDate"]`).type('2024-06-09T08:15');
      cy.get(`[data-cy="paymentPaidDate"]`).blur();
      cy.get(`[data-cy="paymentPaidDate"]`).should('have.value', '2024-06-09T08:15');

      cy.get(`[data-cy="paymentAmount"]`).type('6285.67');
      cy.get(`[data-cy="paymentAmount"]`).should('have.value', '6285.67');

      cy.get(`[data-cy="typeOfPayment"]`).select('CREDIT');

      cy.get(`[data-cy="statusPayment"]`).select('DELAYED');

      cy.get(`[data-cy="customAttributesDetailsJSON"]`).type('suppurate');
      cy.get(`[data-cy="customAttributesDetailsJSON"]`).should('have.value', 'suppurate');
