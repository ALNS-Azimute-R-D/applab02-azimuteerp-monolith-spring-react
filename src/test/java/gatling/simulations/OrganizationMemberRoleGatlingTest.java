...
/**
 * Performance test for the OrganizationMemberRole entity.
 *
 * @see <a href="https://github.com/jhipster/generator-jhipster/tree/v8.5.0/generators/gatling#logging-tips">Logging tips</a>
 */
public class OrganizationMemberRoleGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all organizationMemberRoles")
                ...
                .exec(
                    http("Create new organizationMemberRole")
                        .post("/api/organization-member-roles")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"joinedAt\": \"2020-01-01T00:00:00.000Z\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_organizationMemberRole_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created organizationMemberRole")
                            .get("${new_organizationMemberRole_url}")
                            .headers(headers_http_authenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created organizationMemberRole")
                        .delete("${new_organizationMemberRole_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the OrganizationMemberRole entity").exec(scn);
