...
public class TypeOfOrganizationGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all typeOfOrganizations")
                ...
                .exec(
                    http("Create new typeOfOrganization")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_typeOfOrganization_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created typeOfOrganization").get("${new_typeOfOrganization_url}").headers(headers_http_authenticated)
                    ).pause(10)
                )
                .exec(http("Delete created typeOfOrganization").delete("${new_typeOfOrganization_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the TypeOfOrganization entity").exec(scn);
