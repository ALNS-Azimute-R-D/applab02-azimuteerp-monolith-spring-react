...
public class OrderItemGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all orderItems").get("/api/order-items").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new orderItem")
                        .post("/api/order-items")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"quantity\": 0" + ", \"totalPrice\": 0" + ", \"status\": \"AVAILABLE\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created orderItem").get("${new_orderItem_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created orderItem").delete("${new_orderItem_url}").headers(headers_http_authenticated))
                .pause(10)
        );
