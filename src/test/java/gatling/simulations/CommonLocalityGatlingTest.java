...
public class CommonLocalityGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all commonLocalities").get("/api/common-localities").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new commonLocality")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_commonLocality_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created commonLocality").get("${new_commonLocality_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created commonLocality").delete("${new_commonLocality_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the CommonLocality entity").exec(scn);
