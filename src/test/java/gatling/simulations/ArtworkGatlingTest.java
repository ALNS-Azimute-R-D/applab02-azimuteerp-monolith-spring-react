...
public class ArtworkGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all artworks").get("/api/artworks").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new artwork")
                        .post("/api/artworks")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"artworkTitle\": \"SAMPLE_TEXT\"" +
                                ", \"productionYear\": 0" +
                                ", \"seasonNumber\": 0" +
                                ", \"episodeOrSequenceNumber\": 0" +
                                ", \"registerIdInIMDB\": \"SAMPLE_TEXT\"" +
                                ", \"assetsCollectionUUID\": \"SAMPLE_TEXT\"" +
                                ", \"contentDetailsJSON\": \"SAMPLE_TEXT\"" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_artwork_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created artwork").get("${new_artwork_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created artwork").delete("${new_artwork_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Artwork entity").exec(scn);
