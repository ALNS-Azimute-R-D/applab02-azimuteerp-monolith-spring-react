...
public class CustomerGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all customers").get("/api/customers").headers(headers_http_authenticated).check(status().is(200)))
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created customer").get("${new_customer_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created customer").delete("${new_customer_url}").headers(headers_http_authenticated))
                .pause(10)
        );
