 ...
 */
public class OrganizationGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all organizations").get("/api/organizations").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new organization")
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created organization").get("${new_organization_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created organization").delete("${new_organization_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Organization entity").exec(scn);
