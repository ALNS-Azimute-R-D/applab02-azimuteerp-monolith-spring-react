...
/**
 * Performance test for the OrganizationAttribute entity.
 *
 * @see <a href="https://github.com/jhipster/generator-jhipster/tree/v8.5.0/generators/gatling#logging-tips">Logging tips</a>
 */
public class OrganizationAttributeGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all organizationAttributes")
                ...
                .exec(
                    http("Create new organizationAttribute")
                        .post("/api/organization-attributes")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"attributeName\": \"SAMPLE_TEXT\"" + ", \"attributeValue\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_organizationAttribute_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created organizationAttribute")
                            .get("${new_organizationAttribute_url}")
                            .headers(headers_http_authenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created organizationAttribute")
                        .delete("${new_organizationAttribute_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the OrganizationAttribute entity").exec(scn);
