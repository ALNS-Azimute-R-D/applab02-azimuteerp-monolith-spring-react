    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(http("Get all categories").get("/api/categories").headers(headers_http_authenticated).check(status().is(200)))
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created category").get("${new_category_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created category").delete("${new_category_url}").headers(headers_http_authenticated))
                .pause(10)
        );
