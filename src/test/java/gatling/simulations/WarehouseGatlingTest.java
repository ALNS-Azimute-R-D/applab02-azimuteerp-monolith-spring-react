...
public class WarehouseGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all warehouses").get("/api/warehouses").headers(headers_http_authenticated).check(status().is(200)))
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created warehouse").get("${new_warehouse_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created warehouse").delete("${new_warehouse_url}").headers(headers_http_authenticated))
                .pause(10)
        );
