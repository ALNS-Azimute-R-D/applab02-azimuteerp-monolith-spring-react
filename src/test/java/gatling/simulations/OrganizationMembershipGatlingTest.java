...
/**
 * Performance test for the OrganizationMembership entity.
 *
 * @see <a href="https://github.com/jhipster/generator-jhipster/tree/v8.5.0/generators/gatling#logging-tips">Logging tips</a>
 */
public class OrganizationMembershipGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all organizationMemberships")
                ...
                .exec(
                    http("Create new organizationMembership")
                        .post("/api/organization-memberships")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"joinedAt\": \"2020-01-01T00:00:00.000Z\"" + ", \"activationStatus\": \"INACTIVE\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_organizationMembership_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created organizationMembership")
                            .get("${new_organizationMembership_url}")
                            .headers(headers_http_authenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created organizationMembership")
                        .delete("${new_organizationMembership_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the OrganizationMembership entity").exec(scn);
