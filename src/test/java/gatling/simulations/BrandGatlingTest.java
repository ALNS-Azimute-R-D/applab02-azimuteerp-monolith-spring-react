 ...
 */
public class BrandGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all brands").get("/api/brands").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new brand")
                        .post("/api/brands")
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created brand").get("${new_brand_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created brand").delete("${new_brand_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Brand entity").exec(scn);
