...
public class OrganizationDomainGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all organizationDomains")
                ...
                .exec(
                    http("Create new organizationDomain")
                        .post("/api/organization-domains")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"domainAcronym\": \"SAMPLE_TEXT\"" +
                                ", \"name\": \"SAMPLE_TEXT\"" +
                                ", \"isVerified\": null" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_organizationDomain_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created organizationDomain").get("${new_organizationDomain_url}").headers(headers_http_authenticated)
                    ).pause(10)
                )
                .exec(http("Delete created organizationDomain").delete("${new_organizationDomain_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the OrganizationDomain entity").exec(scn);
