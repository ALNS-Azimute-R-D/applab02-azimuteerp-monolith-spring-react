 ...
 */
public class ActivityGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all activities").get("/api/activities").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new activity")
                        .post("/api/activities")
                        ...
                        .body(
                            StringBody(
                                ...
                                ", \"fullDescription\": \"SAMPLE_TEXT\"" +
                                ", \"mainGoals\": \"SAMPLE_TEXT\"" +
                                ", \"estimatedDurationTime\": null" +
                                ", \"lastPerformedDate\": \"2020-01-01T00:00:00.000Z\"" +
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created activity").get("${new_activity_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created activity").delete("${new_activity_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Activity entity").exec(scn);
