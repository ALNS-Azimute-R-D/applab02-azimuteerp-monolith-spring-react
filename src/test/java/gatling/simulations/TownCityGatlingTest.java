...
public class TownCityGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all townCities").get("/api/town-cities").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new townCity")
                        .post("/api/town-cities")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" + "\"acronym\": \"SAMPLE_TEXT\"" + ", \"name\": \"SAMPLE_TEXT\"" + ", \"geoPolygonArea\": null" + "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_townCity_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created townCity").get("${new_townCity_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created townCity").delete("${new_townCity_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the TownCity entity").exec(scn);
