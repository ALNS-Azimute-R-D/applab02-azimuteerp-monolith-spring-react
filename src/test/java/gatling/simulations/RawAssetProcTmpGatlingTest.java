...
public class RawAssetProcTmpGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all rawAssetProcTmps").get("/api/raw-asset-proc-tmps").headers(headers_http_authenticated).check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new rawAssetProcTmp")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_rawAssetProcTmp_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(http("Get created rawAssetProcTmp").get("${new_rawAssetProcTmp_url}").headers(headers_http_authenticated)).pause(
                        10
                    )
                )
                .exec(http("Delete created rawAssetProcTmp").delete("${new_rawAssetProcTmp_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the RawAssetProcTmp entity").exec(scn);
