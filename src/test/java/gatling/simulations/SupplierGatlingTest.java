...
public class SupplierGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all suppliers").get("/api/suppliers").headers(headers_http_authenticated).check(status().is(200)))
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created supplier").get("${new_supplier_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created supplier").delete("${new_supplier_url}").headers(headers_http_authenticated))
                .pause(10)
        );
