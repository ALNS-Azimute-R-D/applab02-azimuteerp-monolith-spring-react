...
public class TypeOfArtmediaGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all typeOfArtmedias").get("/api/type-of-artmedias").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new typeOfArtmedia")
                        .post("/api/type-of-artmedias")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_typeOfArtmedia_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created typeOfArtmedia").get("${new_typeOfArtmedia_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created typeOfArtmedia").delete("${new_typeOfArtmedia_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the TypeOfArtmedia entity").exec(scn);
