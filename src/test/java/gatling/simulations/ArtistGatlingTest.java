 ...
 */
public class ArtistGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all artists").get("/api/artists").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new artist")
                        .post("/api/artists")
                        ...
                        .body(
                            StringBody(
                                "{" +
                                "\"acronym\": \"SAMPLE_TEXT\"" +
                                ", \"publicName\": \"SAMPLE_TEXT\"" +
                                ", \"realName\": \"SAMPLE_TEXT\"" +
                                ", \"biographyDetailsJSON\": \"SAMPLE_TEXT\"" +
                                "}"
                            )
                        )
                        .asJson()
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created artist").get("${new_artist_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created artist").delete("${new_artist_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Artist entity").exec(scn);
