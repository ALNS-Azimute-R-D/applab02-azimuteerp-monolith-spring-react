...
public class TicketPurchasedGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all ticketPurchaseds").get("/api/ticket-purchaseds").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new ticketPurchased")
                        .post("/api/ticket-purchaseds")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"buyingCode\": \"SAMPLE_TEXT\"" +
                                ", \"duePaymentDate\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"amountOfTickets\": 0" +
                                ", \"taxValue\": 0" +
                                ", \"ticketValue\": 0" +
                                ", \"acquiredSeatsNumbers\": \"SAMPLE_TEXT\"" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_ticketPurchased_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(http("Get created ticketPurchased").get("${new_ticketPurchased_url}").headers(headers_http_authenticated)).pause(
                        10
                    )
                )
                .exec(http("Delete created ticketPurchased").delete("${new_ticketPurchased_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the TicketPurchased entity").exec(scn);
