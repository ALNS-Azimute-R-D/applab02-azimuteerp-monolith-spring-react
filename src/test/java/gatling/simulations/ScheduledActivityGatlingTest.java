...
public class ScheduledActivityGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all scheduledActivities")
                ...
                .exec(
                    http("Create new scheduledActivity")
                        .post("/api/scheduled-activities")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"customizedName\": \"SAMPLE_TEXT\"" +
                                ", \"startTime\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"endTime\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"numberOfAttendees\": 0" +
                                ", \"averageEvaluationInStars\": 0" +
                                ", \"customAttributtesDetailsJSON\": \"SAMPLE_TEXT\"" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_scheduledActivity_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created scheduledActivity").get("${new_scheduledActivity_url}").headers(headers_http_authenticated)
                    ).pause(10)
                )
                .exec(http("Delete created scheduledActivity").delete("${new_scheduledActivity_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the ScheduledActivity entity").exec(scn);
