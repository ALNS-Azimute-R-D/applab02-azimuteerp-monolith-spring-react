...
public class ArticleGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all articles").get("/api/articles").headers(headers_http_authenticated).check(status().is(200)))
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created article").get("${new_article_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created article").delete("${new_article_url}").headers(headers_http_authenticated))
                .pause(10)
        );
