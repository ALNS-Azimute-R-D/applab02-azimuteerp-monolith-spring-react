...
public class ArtisticGenreGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all artisticGenres").get("/api/artistic-genres").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new artisticGenre")
                        .post("/api/artistic-genres")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_artisticGenre_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created artisticGenre").get("${new_artisticGenre_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created artisticGenre").delete("${new_artisticGenre_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the ArtisticGenre entity").exec(scn);
