 ...
 */
public class BusinessUnitGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all businessUnits").get("/api/business-units").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new businessUnit")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_businessUnit_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created businessUnit").get("${new_businessUnit_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created businessUnit").delete("${new_businessUnit_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the BusinessUnit entity").exec(scn);
