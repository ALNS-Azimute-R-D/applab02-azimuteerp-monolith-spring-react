...
public class TypeOfActivityGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all typeOfActivities").get("/api/type-of-activities").headers(headers_http_authenticated).check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new typeOfActivity")
                        .post("/api/type-of-activities")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_typeOfActivity_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created typeOfActivity").get("${new_typeOfActivity_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created typeOfActivity").delete("${new_typeOfActivity_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the TypeOfActivity entity").exec(scn);
