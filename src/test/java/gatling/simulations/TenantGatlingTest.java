...
public class TenantGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all tenants").get("/api/tenants").headers(headers_http_authenticated).check(status().is(200)))
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created tenant").get("${new_tenant_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created tenant").delete("${new_tenant_url}").headers(headers_http_authenticated))
                .pause(10)
        );
