...
public class TypeOfVenueGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all typeOfVenues").get("/api/type-of-venues").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new typeOfVenue")
                        .post("/api/type-of-venues")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_typeOfVenue_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created typeOfVenue").get("${new_typeOfVenue_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created typeOfVenue").delete("${new_typeOfVenue_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the TypeOfVenue entity").exec(scn);
