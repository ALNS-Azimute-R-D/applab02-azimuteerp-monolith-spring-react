...
public class TypeOfEventGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all typeOfEvents").get("/api/type-of-events").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new typeOfEvent")
                        .post("/api/type-of-events")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_typeOfEvent_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created typeOfEvent").get("${new_typeOfEvent_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created typeOfEvent").delete("${new_typeOfEvent_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the TypeOfEvent entity").exec(scn);
