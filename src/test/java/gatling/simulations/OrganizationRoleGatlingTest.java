...
public class OrganizationRoleGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all organizationRoles").get("/api/organization-roles").headers(headers_http_authenticated).check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new organizationRole")
                        .post("/api/organization-roles")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"roleName\": \"SAMPLE_TEXT\"" + ", \"activationStatus\": \"INACTIVE\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_organizationRole_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(http("Get created organizationRole").get("${new_organizationRole_url}").headers(headers_http_authenticated)).pause(
                        10
                    )
                )
                .exec(http("Delete created organizationRole").delete("${new_organizationRole_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the OrganizationRole entity").exec(scn);
