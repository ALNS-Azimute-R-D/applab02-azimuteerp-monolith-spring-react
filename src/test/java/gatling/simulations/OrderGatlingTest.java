...
public class OrderGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all orders").get("/api/orders").headers(headers_http_authenticated).check(status().is(200)))
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created order").get("${new_order_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created order").delete("${new_order_url}").headers(headers_http_authenticated))
                .pause(10)
        );
