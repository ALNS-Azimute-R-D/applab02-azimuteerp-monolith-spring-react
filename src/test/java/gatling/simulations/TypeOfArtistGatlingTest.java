...
public class TypeOfArtistGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all typeOfArtists").get("/api/type-of-artists").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new typeOfArtist")
                        .post("/api/type-of-artists")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_typeOfArtist_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created typeOfArtist").get("${new_typeOfArtist_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created typeOfArtist").delete("${new_typeOfArtist_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the TypeOfArtist entity").exec(scn);
