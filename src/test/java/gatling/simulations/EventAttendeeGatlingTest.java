...
public class EventAttendeeGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all eventAttendees").get("/api/event-attendees").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new eventAttendee")
                        .post("/api/event-attendees")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"attendedAsRole\": \"SAMPLE_TEXT\"" +
                                ", \"wasPresentInEvent\": null" +
                                ", \"shouldBuyTicket\": null" +
                                ", \"ticketNumber\": \"SAMPLE_TEXT\"" +
                                ", \"seatNumber\": \"SAMPLE_TEXT\"" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_eventAttendee_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created eventAttendee").get("${new_eventAttendee_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created eventAttendee").delete("${new_eventAttendee_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the EventAttendee entity").exec(scn);
