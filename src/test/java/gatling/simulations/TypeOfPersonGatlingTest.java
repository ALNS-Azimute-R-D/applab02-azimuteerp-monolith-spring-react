...
public class TypeOfPersonGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all typeOfPeople").get("/api/type-of-people").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new typeOfPerson")
                        .post("/api/type-of-people")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"code\": \"SAMPLE_TEXT\"" + ", \"description\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_typeOfPerson_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created typeOfPerson").get("${new_typeOfPerson_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created typeOfPerson").delete("${new_typeOfPerson_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the TypeOfPerson entity").exec(scn);
