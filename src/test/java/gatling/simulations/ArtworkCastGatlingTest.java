...
public class ArtworkCastGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all artworkCasts").get("/api/artwork-casts").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new artworkCast")
                        .post("/api/artwork-casts")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"orderOfAppearance\": 0" +
                                ", \"characterName\": \"SAMPLE_TEXT\"" +
                                ", \"mainAssetUUID\": \"SAMPLE_TEXT\"" +
                                ", \"characterDetailsJSON\": \"SAMPLE_TEXT\"" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_artworkCast_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created artworkCast").get("${new_artworkCast_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created artworkCast").delete("${new_artworkCast_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the ArtworkCast entity").exec(scn);
