...
public class EventProgramGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all eventPrograms").get("/api/event-programs").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new eventProgram")
                        .post("/api/event-programs")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"percentageExecution\": 0" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_eventProgram_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created eventProgram").get("${new_eventProgram_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created eventProgram").delete("${new_eventProgram_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the EventProgram entity").exec(scn);
