...
public class PaymentGatewayGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all paymentGateways").get("/api/payment-gateways").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new paymentGateway")
                        .post("/api/payment-gateways")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"aliasCode\": \"SAMPLE_TEXT\"" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_paymentGateway_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created paymentGateway").get("${new_paymentGateway_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created paymentGateway").delete("${new_paymentGateway_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the PaymentGateway entity").exec(scn);
