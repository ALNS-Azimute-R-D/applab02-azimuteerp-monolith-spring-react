...
public class AssetMetadataGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all assetMetadata").get("/api/asset-metadata").headers(headers_http_authenticated).check(status().is(200)))
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created assetMetadata").get("${new_assetMetadata_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created assetMetadata").delete("${new_assetMetadata_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the AssetMetadata entity").exec(scn);
