...
import static org.dexterity.darueira.azimuteerp.monolith.springreact.domain.OrderItemAsserts.*;
...
import org.dexterity.darueira.azimuteerp.monolith.springreact.domain.enumeration.OrderItemStatusEnum;
import org.dexterity.darueira.azimuteerp.monolith.springreact.repository.OrderItemRepository;
import org.dexterity.darueira.azimuteerp.monolith.springreact.service.OrderItemService;
import org.dexterity.darueira.azimuteerp.monolith.springreact.service.dto.OrderItemDTO;
import org.dexterity.darueira.azimuteerp.monolith.springreact.service.mapper.OrderItemMapper;
...
class OrderItemResourceIT {
    ...
    void createOrderItem() throws Exception {
        ...
        var returnedOrderItemDTO = om.readValue(
            restOrderItemMockMvc
                .perform(
                    post(ENTITY_API_URL).with(csrf()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(orderItemDTO))
                )
    ...
    void createOrderItemWithExistingId() throws Exception {
        ...
        restOrderItemMockMvc
            .perform(post(ENTITY_API_URL).with(csrf()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(orderItemDTO)))
    ...
    void putWithMissingIdPathParamOrderItem() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restOrderItemMockMvc
            .perform(put(ENTITY_API_URL).with(csrf()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(orderItemDTO)))
    ...
    void patchWithMissingIdPathParamOrderItem() throws Exception {
        ...
        restOrderItemMockMvc
            .perform(
                patch(ENTITY_API_URL).with(csrf()).contentType("application/merge-patch+json").content(om.writeValueAsBytes(orderItemDTO))
            )
            .andExpect(status().isMethodNotAllowed());
