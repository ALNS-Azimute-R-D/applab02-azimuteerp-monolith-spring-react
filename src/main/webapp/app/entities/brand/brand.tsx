  ...
  return (
    <div>
      <h2 id="brand-heading" data-cy="BrandHeading">
        <Translate contentKey="azimuteErpSpringReactMonolith03App.brand.home.title">Brands</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="azimuteErpSpringReactMonolith03App.brand.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/brand/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="azimuteErpSpringReactMonolith03App.brand.home.createLabel">Create new Brand</Translate>
          </Link>
        </div>
      </h2>
        ...
        {brandList && brandList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="azimuteErpSpringReactMonolith03App.brand.id">ID</Translate>{' '}
                ...
                <th className="hand" onClick={sort('logoBrand')}>
                  <Translate contentKey="azimuteErpSpringReactMonolith03App.brand.logoBrand">Logo Brand</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('logoBrand')} />
                </th>
                <th />
              </tr>
            </thead>
              ...
              {brandList.map((brand, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>
                    {brand.logoBrand ? (
                      <div>
                        {brand.logoBrandContentType ? (
                          <a onClick={openFile(brand.logoBrandContentType, brand.logoBrand)}>
                            <img src={`data:${brand.logoBrandContentType};base64,${brand.logoBrand}`} style={{ maxHeight: '30px' }} />
                            &nbsp;
                          </a>
                        ) : null}
                        <span>
                          {brand.logoBrandContentType}, {byteSize(brand.logoBrand)}
                        </span>
                      </div>
                    ) : null}
                  </td>
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button
                        tag={Link}
                        to={`/brand/${brand.id}/edit?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="primary"
                        size="sm"
                      ...
                      <Button
                        onClick={() =>
                          (window.location.href = `/brand/${brand.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`)
                        }
                        color="danger"
                        size="sm"
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="azimuteErpSpringReactMonolith03App.brand.home.notFound">No Brands found</Translate>
            </div>
          )
        )}
      </div>
