  ...
  return (
    <div>
      <h2 id="activity-heading" data-cy="ActivityHeading">
        <Translate contentKey="azimuteErpSpringReactMonolith03App.activity.home.title">Activities</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="azimuteErpSpringReactMonolith03App.activity.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/activity/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="azimuteErpSpringReactMonolith03App.activity.home.createLabel">Create new Activity</Translate>
          </Link>
        </div>
      </h2>
        ...
        {activityList && activityList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="azimuteErpSpringReactMonolith03App.activity.id">ID</Translate>{' '}
                ...
                <th className="hand" onClick={sort('mainGoals')}>
                  <Translate contentKey="azimuteErpSpringReactMonolith03App.activity.mainGoals">Main Goals</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('mainGoals')} />
                </th>
                <th className="hand" onClick={sort('estimatedDurationTime')}>
                  <Translate contentKey="azimuteErpSpringReactMonolith03App.activity.estimatedDurationTime">
                    Estimated Duration Time
                  </Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('estimatedDurationTime')} />
                </th>
                <th className="hand" onClick={sort('lastPerformedDate')}>
                  <Translate contentKey="azimuteErpSpringReactMonolith03App.activity.lastPerformedDate">Last Performed Date</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('lastPerformedDate')} />
                </th>
            ...
            <tbody>
              {activityList.map((activity, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{activity.estimatedDurationTime ? <DurationFormat value={activity.estimatedDurationTime} /> : null}</td>
                  <td>
                    {activity.lastPerformedDate ? (
                      <TextFormat type="date" value={activity.lastPerformedDate} format={APP_LOCAL_DATE_FORMAT} />
                    ) : null}
                  </td>
                  <td>{activity.createdAt ? <TextFormat type="date" value={activity.createdAt} format={APP_DATE_FORMAT} /> : null}</td>
                  <td>
                    <Translate contentKey={`azimuteErpSpringReactMonolith03App.ActivationStatusEnum.${activity.activationStatus}`} />
                  </td>
                  <td>
                    {activity.typeOfActivity ? (
                      <Link to={`/type-of-activity/${activity.typeOfActivity.id}`}>{activity.typeOfActivity.acronym}</Link>
                    ) : (
                      ''
                    )}
                  </td>
                  <td>
                    {activity.createdByUser ? (
                      <Link to={`/person/${activity.createdByUser.id}`}>{activity.createdByUser.fullname}</Link>
                    ) : (
                      ''
                    )}
                  </td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="azimuteErpSpringReactMonolith03App.activity.home.notFound">No Activities found</Translate>
            </div>
          )
        )}
      </div>
