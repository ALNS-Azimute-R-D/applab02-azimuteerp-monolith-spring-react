  ...
  return (
    <div>
      <h2 id="order-heading" data-cy="OrderHeading">
        <Translate contentKey="azimuteErpSpringReactMonolith03App.order.home.title">Orders</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="azimuteErpSpringReactMonolith03App.order.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/order/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="azimuteErpSpringReactMonolith03App.order.home.createLabel">Create new Order</Translate>
          </Link>
        </div>
      </h2>
        ...
        {orderList && orderList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="azimuteErpSpringReactMonolith03App.order.id">ID</Translate>{' '}
                ...
                <th className="hand" onClick={sort('totalTaxValue')}>
                  <Translate contentKey="azimuteErpSpringReactMonolith03App.order.totalTaxValue">Total Tax Value</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('totalTaxValue')} />
                </th>
                <th className="hand" onClick={sort('totalDueValue')}>
                  <Translate contentKey="azimuteErpSpringReactMonolith03App.order.totalDueValue">Total Due Value</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('totalDueValue')} />
                </th>
            ...
            <tbody>
              {orderList.map((order, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>
                    <Translate contentKey={`azimuteErpSpringReactMonolith03App.OrderStatusEnum.${order.status}`} />
                  </td>
                  <td>
                    {order.estimatedDeliveryDate ? (
                      <TextFormat type="date" value={order.estimatedDeliveryDate} format={APP_DATE_FORMAT} />
                    ) : null}
                  </td>
                  <td>
                    <Translate contentKey={`azimuteErpSpringReactMonolith03App.ActivationStatusEnum.${order.activationStatus}`} />
                  </td>
                  <td>{order.invoice ? <Link to={`/invoice/${order.invoice.id}`}>{order.invoice.description}</Link> : ''}</td>
                  <td>{order.customer ? <Link to={`/customer/${order.customer.id}`}>{order.customer.fullname}</Link> : ''}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="azimuteErpSpringReactMonolith03App.order.home.notFound">No Orders found</Translate>
            </div>
          )
        )}
      </div>
