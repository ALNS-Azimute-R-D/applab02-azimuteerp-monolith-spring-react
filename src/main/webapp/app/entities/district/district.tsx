  ...
  return (
    <div>
      <h2 id="district-heading" data-cy="DistrictHeading">
        <Translate contentKey="azimuteErpSpringReactMonolith03App.district.home.title">Districts</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="azimuteErpSpringReactMonolith03App.district.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/district/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="azimuteErpSpringReactMonolith03App.district.home.createLabel">Create new District</Translate>
          </Link>
        </div>
      </h2>
        ...
        {districtList && districtList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="azimuteErpSpringReactMonolith03App.district.id">ID</Translate>{' '}
            ...
            <tbody>
              {districtList.map((district, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{district.name}</td>
                  <td>
                    {district.geoPolygonArea ? (
                      <div>
                        {district.geoPolygonAreaContentType ? (
                          <a onClick={openFile(district.geoPolygonAreaContentType, district.geoPolygonArea)}>
                            <Translate contentKey="entity.action.open">Open</Translate>
                            &nbsp;
                          </a>
                        ) : null}
                        <span>
                          {district.geoPolygonAreaContentType}, {byteSize(district.geoPolygonArea)}
                        </span>
                      </div>
                    ) : null}
                  </td>
                  <td>{district.townCity ? <Link to={`/town-city/${district.townCity.id}`}>{district.townCity.name}</Link> : ''}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="azimuteErpSpringReactMonolith03App.district.home.notFound">No Districts found</Translate>
            </div>
          )
        )}
      </div>
