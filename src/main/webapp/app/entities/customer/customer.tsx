  ...
  return (
    <div>
      <h2 id="customer-heading" data-cy="CustomerHeading">
        <Translate contentKey="azimuteErpSpringReactMonolith03App.customer.home.title">Customers</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="azimuteErpSpringReactMonolith03App.customer.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/customer/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="azimuteErpSpringReactMonolith03App.customer.home.createLabel">Create new Customer</Translate>
          </Link>
        </div>
      </h2>
        ...
        {customerList && customerList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="azimuteErpSpringReactMonolith03App.customer.id">ID</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                <th className="hand" onClick={sort('customerBusinessCode')}>
                  <Translate contentKey="azimuteErpSpringReactMonolith03App.customer.customerBusinessCode">
            ...
            <tbody>
              {customerList.map((customer, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>
                    <Translate contentKey={`azimuteErpSpringReactMonolith03App.CustomerStatusEnum.${customer.customerStatus}`} />
                  </td>
                  <td>
                    <Translate contentKey={`azimuteErpSpringReactMonolith03App.ActivationStatusEnum.${customer.activationStatus}`} />
                  </td>
                  <td>
                    {customer.buyerPerson ? <Link to={`/person/${customer.buyerPerson.id}`}>{customer.buyerPerson.lastname}</Link> : ''}
                  </td>
                  <td>
                    {customer.customerType ? (
                      <Link to={`/customer-type/${customer.customerType.id}`}>{customer.customerType.name}</Link>
                    ) : (
                      ''
                    )}
                  </td>
                  <td>{customer.district ? <Link to={`/district/${customer.district.id}`}>{customer.district.name}</Link> : ''}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="azimuteErpSpringReactMonolith03App.customer.home.notFound">No Customers found</Translate>
            </div>
          )
        )}
      </div>
