  ...
  return (
    <div>
      <h2 id="event-heading" data-cy="EventHeading">
        <Translate contentKey="azimuteErpSpringReactMonolith03App.event.home.title">Events</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="azimuteErpSpringReactMonolith03App.event.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/event/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="azimuteErpSpringReactMonolith03App.event.home.createLabel">Create new Event</Translate>
          </Link>
        </div>
      </h2>
        ...
        {eventList && eventList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="azimuteErpSpringReactMonolith03App.event.id">ID</Translate>{' '}
                ...
                <th className="hand" onClick={sort('defaultTicketValue')}>
                  <Translate contentKey="azimuteErpSpringReactMonolith03App.event.defaultTicketValue">Default Ticket Value</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('defaultTicketValue')} />
                </th>
            ...
            <tbody>
              {eventList.map((event, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>
                    <Translate contentKey={`azimuteErpSpringReactMonolith03App.EventStatusEnum.${event.status}`} />
                  </td>
                  <td>
                    <Translate contentKey={`azimuteErpSpringReactMonolith03App.ActivationStatusEnum.${event.activationStatus}`} />
                  </td>
                  <td>{event.mainVenue ? <Link to={`/venue/${event.mainVenue.id}`}>{event.mainVenue.acronym}</Link> : ''}</td>
                  <td>{event.typeOfEvent ? <Link to={`/type-of-event/${event.typeOfEvent.id}`}>{event.typeOfEvent.acronym}</Link> : ''}</td>
                  <td>
                    {event.promoteurPerson ? <Link to={`/person/${event.promoteurPerson.id}`}>{event.promoteurPerson.fullname}</Link> : ''}
                  </td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="azimuteErpSpringReactMonolith03App.event.home.notFound">No Events found</Translate>
            </div>
          )
        )}
      </div>
