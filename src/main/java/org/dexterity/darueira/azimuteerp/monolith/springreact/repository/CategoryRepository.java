...
public interface CategoryRepository extends JpaRepository<Category, Long> {
    ...
    @Query(
        value = "select category from Category category left join fetch category.categoryParent",
        countQuery = "select count(category) from Category category"
    )
    Page<Category> findAllWithToOneRelationships(Pageable pageable);

    @Query("select category from Category category left join fetch category.categoryParent")
    List<Category> findAllWithToOneRelationships();

    @Query("select category from Category category left join fetch category.categoryParent where category.id =:id")
