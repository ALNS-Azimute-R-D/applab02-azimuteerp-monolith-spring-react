...
public class ProductRepositoryWithBagRelationshipsImpl implements ProductRepositoryWithBagRelationships {
    ...
    public Optional<Product> fetchBagRelationships(Optional<Product> product) {
        return product.map(this::fetchToSuppliers);
    }
    ...
    public List<Product> fetchBagRelationships(List<Product> products) {
        return Optional.of(products).map(this::fetchToSuppliers).orElse(Collections.emptyList());
    }

    Product fetchToSuppliers(Product result) {
        return entityManager
            .createQuery("select product from Product product left join fetch product.toSuppliers where product.id = :id", Product.class)
            .setParameter(ID_PARAMETER, result.getId())
            .getSingleResult();
    }

    List<Product> fetchToSuppliers(List<Product> products) {
        ...
        List<Product> result = entityManager
            .createQuery(
                "select product from Product product left join fetch product.toSuppliers where product in :products",
                Product.class
            )
