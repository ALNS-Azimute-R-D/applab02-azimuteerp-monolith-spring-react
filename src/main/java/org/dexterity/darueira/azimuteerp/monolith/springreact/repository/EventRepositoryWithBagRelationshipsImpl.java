...
public class EventRepositoryWithBagRelationshipsImpl implements EventRepositoryWithBagRelationships {
    ...
    public Optional<Event> fetchBagRelationships(Optional<Event> event) {
        return event.map(this::fetchAssetCollections);
    }
    ...
    public List<Event> fetchBagRelationships(List<Event> events) {
        return Optional.of(events).map(this::fetchAssetCollections).orElse(Collections.emptyList());
    }

    Event fetchAssetCollections(Event result) {
        return entityManager
            .createQuery("select event from Event event left join fetch event.assetCollections where event.id = :id", Event.class)
            .setParameter(ID_PARAMETER, result.getId())
            .getSingleResult();
    }

    List<Event> fetchAssetCollections(List<Event> events) {
        ...
        List<Event> result = entityManager
            .createQuery("select event from Event event left join fetch event.assetCollections where event in :events", Event.class)
