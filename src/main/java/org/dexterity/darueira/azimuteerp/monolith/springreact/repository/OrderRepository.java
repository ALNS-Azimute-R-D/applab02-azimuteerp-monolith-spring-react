...
public interface OrderRepository extends JpaRepository<Order, Long> {
    ...
    @Query(
        value = "select jhiOrder from Order jhiOrder left join fetch jhiOrder.invoice left join fetch jhiOrder.customer",
        countQuery = "select count(jhiOrder) from Order jhiOrder"
    )
    Page<Order> findAllWithToOneRelationships(Pageable pageable);

    @Query("select jhiOrder from Order jhiOrder left join fetch jhiOrder.invoice left join fetch jhiOrder.customer")
    List<Order> findAllWithToOneRelationships();

    @Query("select jhiOrder from Order jhiOrder left join fetch jhiOrder.invoice left join fetch jhiOrder.customer where jhiOrder.id =:id")
