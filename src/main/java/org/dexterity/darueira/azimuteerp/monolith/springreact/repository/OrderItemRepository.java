...
public interface OrderItemRepository extends JpaRepository<OrderItem, Long> {
    ...
    @Query(
        value = "select orderItem from OrderItem orderItem left join fetch orderItem.article left join fetch orderItem.order",
        countQuery = "select count(orderItem) from OrderItem orderItem"
    )
    Page<OrderItem> findAllWithToOneRelationships(Pageable pageable);

    @Query("select orderItem from OrderItem orderItem left join fetch orderItem.article left join fetch orderItem.order")
    List<OrderItem> findAllWithToOneRelationships();

    @Query(
        "select orderItem from OrderItem orderItem left join fetch orderItem.article left join fetch orderItem.order where orderItem.id =:id"
    )
